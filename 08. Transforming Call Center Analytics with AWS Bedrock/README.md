...
Analyzing and improving call scripts is crucial for enhancing sales conversions in the dynamic and customer-centric world of call centers. The Go3 OTT video service, which operates across the Baltic countries of Lithuania, Latvia, and Estonia, faced a not-unique challenge: their call center needed to review calls made in multiple languages to provide consistent and effective guidelines to their agents. To address this, an AWS-based application was developed leveraging AWS Bedrock to analyze and translate call transcripts, ensuring that the Go3 call center can maintain high-quality interactions and boost sales performance.

## Problem Statement

The Go3 call center handles various customer inquiries across different languages. The call center manager required a solution to analyze real-time calls, translate the transcripts, and provide actionable insights to improve call scripts. The primary challenges included:
...
We leveraged several AWS services, including AWS Bedrock, to tackle these challenges to build a robust solution. The key components of the application included:

- **Amazon S3**: This is for storing the call audio files and transcripts.
- **AWS Lambda**: For orchestrating the process.
- **Amazon Transcribe**: This is used to create the transcription of the call recordings.
...
The solution's architecture is designed to be modular and scalable, leveraging AWS services to their full potential. Here's a high-level overview of the architecture:

1. **Audio Storage**: Call audio files are stored in an S3 bucket in `recordings` directory.
2. **Transcription**: When a new audio file is uploaded, an S3 event triggers a Lambda function that initiates the transcription process, which result is stored in an S3 bucket in `transcribed` directory.
3. **Translation and Analysis**: When the transcribed text is uploaded, the S3 event triggers Lambda again, which initiates the AWS Bedrock to convert it into the desired language and analyze the content. The result is saved on S3 in `output` directory.
4. **Sendout of Result**: When the results are uploaded, again the S3 event triggers the Lambda, which sends out the result to the SNS topic.
5. **Review**: The translated transcript and analysis results are emailed for the review.

The architecture of the solution is presented on the below diagram.

![Architecture](./images/architecture.drawio.png)

Note that there are potentially two possible solutions: using a bucket for all the operations or one bucket per operation. The first solution makes it cleaner regarding the S3 resources (only a single bucket is created for the application). However, due to the limitations of the S3 events, it can work only with one Lambda function, which has to perform all the tasks depending on the directory in which the file is created.

Another solution would be to use several buckets and separate Lambda for each operation. This would be cleaner from the Lambda code standpoint, but as this is hidden from the user, I have decided to use a single-bucket approach.

In the above diagram, there are three instances of the bucket and lambda; however, they are all implemented as one bucket with several directories and one Lambda with the handler dispatching the requests.
...
This function is the main entry point for the AWS Lambda function that processes files uploaded to an S3 bucket. S3 object creation events trigger it and performs the following steps:
...
The function returns an error response if the input file is not an MP3 file. If any errors are creating the Transcribe client or starting the transcription job, the function returns an error response.

Upon successfully initiating the transcription job, the function returns a successful response.


### Analyze

Analyze a transcript file stored in an S3 bucket using Amazon Bedrock. It also uses a subfunction to format the content of a transcription result from Amazon Transcribe.
...
The first function takes a dictionary containing the results of an Amazon Transcribe job and formats the content into a human-readable format. The function loops through the individual items in the transcription result, grouping the content by the speaker and adding timestamps and labels to each line.
...
The instrunctions for the Amazon Bedrock model are kept in a separate module `instructions.py`, to make it easier to modify them.
...
This helper function sends the result or an error to the SNS topic.
...
If the message is successfully published on the SNS topic, the function returns a successful response.

### Notify

Send the notification with the results of the analyses.
...
This template defines resources for an AWS serverless application modeled using the AWS Serverless Application Model (SAM). It specifies a collection of AWS resources, properties, and relationships.
...
1. **S3 Bucket Policy** (`InputBucketBucketPolicy`): Creates a bucket policy on `InputBucket` that denies any non-secure transport requests to the bucket, enforcing encryption in transit.

2. **SNS Topic** (`OutputNotificationMetricFilterTopic`): Defines an Amazon SNS topic for posting notifications, named according to the stack name and account ID. Subscriptions can be made to notify endpoints such as the email of the `Owner`.

3. **IAM Role** (`AnalyzeFunctionRole`): Specifies an IAM role assumed by the Lambda function `AnalyzeFunction`. It grants permissions for accessing AWS services like S3, Transcribe, and for publishing messages to SNS topics.

4. **CloudWatch Log Group** (`AnalyzeFunctionLogGroup`): Establishes a log group in Amazon CloudWatch Logs for storing logs from `AnalyzeFunction`.

5. **Lambda Function** (`AnalyzeFunction`): Deploys an AWS Lambda function with Python runtime, configured with appropriate memory size, timeout settings, and environment variables. Its associated IAM role permits actions on S3 and the SNS topic.

6. **Lambda Permission** (`AnalyzeFunctionInputBucketPermission`): Assigns permission for `AnalyzeFunction` Lambda function to be invoked by events originating from `InputBucket`.

7. **IAM Role** (`ApplyNotificationFunctionRole`): Allocates an IAM role to be assumed by the Lambda function `ApplyBucketNotificationFunction`, which is responsible for applying bucket notifications. It specifically allows updating the S3 bucket notification configuration.

8. **CloudWatch Log Group** (`ApplyBucketNotificationFunctionLogGroup`): Sets up a log group in Amazon CloudWatch Logs for the `ApplyBucketNotificationFunction`, including retention policies.

9.  **Lambda Function** (`ApplyBucketNotificationFunction`): Implements a Lambda function with inline Python code designed to add a bucket notification configuration to `InputBucket`. This function addresses circular dependencies between the Lambda function and the S3 bucket.

10. **Custom Resource** (`ApplyNotification`): Activates `ApplyBucketNotificationFunction` to establish the intended S3 bucket notification linkage between `InputBucket` and `AnalyzeFunction`.

11. **Outputs**: Provides key references to resources created by the template, available for usage in other stacks or applications.
...
2. **Clone the Repository**: Download the project files by cloning the GitHub repository:
...
5. **Test**: Optionally test the build. Before update the `events\` and `environments\` json files with the arns of your test resources and run the following:
...
6. **Deploy**: If the `Owner` parameter has no default value, you need to specify it here:

   ```sh
   sam deploy --parameter-overrides Owner={your-email-address}
   ```

    Otherwise, deploy without any parameters:

   ```sh
   sam deploy
   ```

7. **Accept the SNS Notifications**: An email requesting you to accept the notifications should be in your inbox. Accept it.

8. **Verify Deployment**: Upload an mp3 file with the call recording to the `InputBucket` into `recordings` directory.
...
We are considering the following next steps in application development. If you have other ideas, please share.
